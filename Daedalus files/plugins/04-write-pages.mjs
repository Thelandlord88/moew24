...
export default {
  ...
  async eachNode(ctx, { service, suburb }) {
    ...
    // Prefer neighbors chosen by the policy engine (05-internal-links)
    const chosen = ctx._neighborsMap?.[`${service}/${suburb}`];
    const neighbors = Array.isArray(chosen) && chosen.length
      ? chosen.slice(0, max)
      : (ctx.datasets.adj[suburb] || []).slice(0, max);
...
`;
    ...
    ctx.reports.pages ||= [];
    ctx.reports.pages.push({ service, suburb, path: `${outDir}/index.astro`, neighbors: neighbors.length });
  }
};
