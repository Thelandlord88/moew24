...
export async function createContext({ args = {} }) {
  ...
  // Determine suburb keys: prefer explicit mapping, else fall back to adjacency keys
  let suburbKeys;
  if (clusters.suburbs && typeof clusters.suburbs === 'object') {
    suburbKeys = Object.keys(clusters.suburbs);
  } else if (adj && typeof adj === 'object') {
    suburbKeys = Object.keys(adj);
  } else {
    suburbKeys = Object.keys(clusters);
  }
