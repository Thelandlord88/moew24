...
# ---------------------------
# 8) Astro SSR Detection (NoAdapterInstalled debugging)
# ---------------------------
section "Astro SSR Patterns & Deep Triggers"

# Astro SSR patterns in components
ASTRO_SSR_MATCHES=$(\
  { rg -n "prerender\s*=\s*false" "${RG_EXCL[@]}" -g '**/*.astro' || true; } ;\
  { rg -n "Astro\.request|Astro\.locals" "${RG_EXCL[@]}" -g '**/*.astro' || true; } ;\
  { rg -n "from\s+['\"]~/server/" "${RG_EXCL[@]}" -g '**/*.astro' || true; } \
)

# Deep SSR triggers
DEEP_SSR_MATCHES=$(\
  { rg -n "@astrojs/(netlify|vercel|node|cloudflare)" "${RG_EXCL[@]}" || true; } ;\
  { rg -n "USE_SSR|ENABLE_SSR|SSR_MODE|NETLIFY.*=.*1|VERCEL.*=.*1" "${RG_EXCL[@]}" || true; } ;\
  { rg -n "new\s+Response\s*\(|new\s+Request\s*\(" "${RG_EXCL[@]}" -g '**/*.astro' || true; } ;\
  { rg -n "process\.env\.(ASTRO_|SSR_|SERVER_|NETLIFY_|VERCEL_)" "${RG_EXCL[@]}" || true; } \
)

ALL_SSR_MATCHES="${ASTRO_SSR_MATCHES}${ASTRO_SSR_MATCHES:+$'\n'}${DEEP_SSR_MATCHES}"

if [[ -n "$ALL_SSR_MATCHES" ]]; then
  CNT_SSR=$(printf '%s\n' "$ALL_SSR_MATCHES" | wc -l | tr -d ' ')
  warn "potential SSR triggers found: $CNT_SSR (NoAdapterInstalled debugging)"
  printf '%s\n' "$ALL_SSR_MATCHES" | head -n "$MAX_RESULTS" | sed 's/^/  • /'
  echo "  📋 For SSR debugging: check output:'static' in astro.config.mjs"
else
  okay "no obvious SSR triggers detected (good for static builds)"
fi

declare -i CNT_SSR_TOTAL=${CNT_SSR:-0}
...
printf "${BOLD}SSR triggers found:${RST} %d\n" "$CNT_SSR_TOTAL"
