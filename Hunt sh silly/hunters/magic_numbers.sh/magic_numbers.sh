...
for domain in "${!MAGIC_PATTERNS[@]}"; do
    ...
    while IFS= read -r line; do
        if [[ -n "$line" ]]; then
            ...
            if [[ "$file" == *"node_modules"* ]] || [[ "$file" == *".min."* ]]; then
                continue
            fi
            ...
            if [[ $TOTAL_MAGIC_COUNT -le 15 ]]; then
                echo "    ⚠️  $domain in $file:$line_num"
                echo "       → $(echo "$content" | xargs | cut -c1-80)"
            fi
        fi
    done < <(grep -r -E "$pattern" src/ --include="*.ts" --include="*.js" --include="*.astro" -n 2>/dev/null | head -20 || true)
...
if [[ $TOTAL_MAGIC_COUNT -gt 15 ]]; then
...
done < <(grep -r -E '[0-9]+' src/ --include="*.ts" --include="*.js" --include="*.astro" 2>/dev/null | head -100 || true)
...
for pattern in "${BUSINESS_PATTERNS[@]}"; do
    ...
    count=$(grep -r -E "$pattern" src/ --include="*.ts" --include="*.js" --include="*.astro" 2>/dev/null | wc -l || echo "0")
    if [[ $count -gt 0 ]]; then
...
if [[ ${#MAGIC_INSTANCES[@]} -gt 0 ]]; then
    MAGIC_INSTANCES_JSON=$(printf '%s\n' "${MAGIC_INSTANCES[@]}" | head -50 | jq -R 'split(":") | {domain: .[0], file: .[1], line: (.[2] | tonumber), content: (.[3:] | join(":"))}' | jq -s .)
fi

DOMAIN_COUNTS_JSON="{}"
if [[ ${#DOMAIN_COUNTS[@]} -gt 0 ]]; then
    DOMAIN_COUNTS_JSON=$(for domain in "${!DOMAIN_COUNTS[@]}"; do
        echo "\"$domain\": ${DOMAIN_COUNTS[$domain]}"
    done | jq -s 'map(split(": ") | {key: .[0], value: (.[1] | tonumber)}) | from_entries')
fi
