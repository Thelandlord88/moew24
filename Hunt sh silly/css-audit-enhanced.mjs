...
const DEFAULT_CONFIG = {
  srcGlobs: ['src/**/*.{astro,html,js,jsx,ts,tsx,mjs,mdx}'],
  cssGlobs: ['src/**/*.css'],
  semanticPrefixes: ['btn','badge','label','card','nav','pill','chip','link-brand','text-brand','bg-brand','ring-brand','bar-brand','prose-custom','focus-ring','q-','blog-','review-'],
  allowlist: ['grecaptcha-badge','third-party-widget-*','blog-*','q-*','review-*'],
  denylist: ['tmp-*','todo-*','debug-*'],
  helperFunctions: ['clsx','classnames','twJoin','twMerge','cva','tv','cx'],
  ciThresholds: { suspicious: 50, missingDefinitions: 0, unusedCustom: 9999, bannedTokens: 0 },
};
  ...
  try {
    ...
    function walk(dir) {
      ...
      for (const e of entries) {
        if (e.name.startsWith('.') || e.name === 'node_modules' || e.name === '__reports' || e.name === '.git') continue;
...
async function loadTailwindContext() {
  ...
  try { resolveConfig = (await import('tailwindcss/resolveConfig')).default; }
  catch { /* optional */ }
...
function isDenylisted(token, cfg) {
  return cfg.denylist.some(rule => rule.endsWith('*') ? token.startsWith(rule.slice(0, -1)) : token === rule);
}
...
(async function main() {
  ...
  for (const [k, v] of Object.entries({
    ...
    bannedTokens: report.counts.bannedTokens
  })) {
...
function renderMarkdown(r) {
  ...
  lines.push(`| Banned tokens | ${r.counts.bannedTokens} |`, '');
  ...
  lines.push('## Banned Tokens', '');
  if (r.bannedTokens && r.bannedTokens.length) {
    r.bannedTokens.slice(0, 500).forEach(tok => {
      const samples = r.occurrences?.[tok]?.slice(0, 3) || [];
      const where = samples.map(s => `- ${s.file}${s.line ? `:${s.line}` : ''}`).join('\n');
      lines.push(`- \`${tok}\``);
      if (where) lines.push(where);
