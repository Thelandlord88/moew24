Below is a **single, paste-and-run installer** that merges the strongest ideas into one upstream-first system:

* idempotent bootstrap with consent flags
* clean Astro 5 + Tailwind v4 pipeline (Vite plugin only)
* geo + services scaffolds (SSG, Nearby, cross-links)
* JSON-LD injector (XSS-safe, UI↔schema lockstep)
* SEO reporter (happy-dom)
* Integrity guards (anchor diversity, no hidden keywords, no UA-based DOM; duplicate-page similarity)
* Geo Doctor (sanity checks, coverage)
* Playwright smoke tests (quote path/JSON-LD presence)
* Netlify config + GitHub Actions CI

> Save as `install-unified.sh`, run with:
> `bash install-unified.sh --force --site=https://onendone.com.au`
...
# What this gives you (and why it converts)

**Psychology & flow (Olivia):**

* Above-the-fold **clarity + trust chips + single CTA** on every conversion page.
* Suburb pages keep **Nearby** high to preserve exploration momentum.
* Service pages show a **short, scannable checklist** and **cross-service** links to prevent dead ends.
* JSON-LD mirrors visible UI (no hidden claims), increasing perceived authenticity.

**UI & rhythm (Marco):**

* Consistent spacing tokens: outer 24px / card 16px / list 12px.
* One primary button color; secondary links are text-style to reduce choice paralysis.
* Mobile sticky CTA keeps the “next step” always reachable.

**Engineering & speed (Ethan):**

* SSG routes with `getStaticPaths()` for suburbs and service/suburb pairs (fast, cacheable).
* Tailwind v4 through `@tailwindcss/vite` only (no redundant PostCSS path).
* XSS-safe JSON-LD injector; canonical links on every page via `BaseLayout`.
* **Geo Doctor** blocks bad data before build; **SEO reporter** catches canonical/JSON-LD errors after build.
* **Guards** enforce anchor diversity, ban hidden keyword blocks & UA-based DOM, and detect near-duplicate suburb pages.
* **Playwright smokes** verify key pages render and JSON-LD exists.

---

## How to adapt

* **Add real data**: replace `src/data/suburbs.json` and `src/data/services.json` with your canonical datasets.
* **Tune guards**: adjust thresholds in `anchors.mjs` and `similarity.mjs` to your content density.
* **Branding**: swap trust chip labels, typography, and button class names to match your palette.
* **Expand services**: append new services in `services.json`; routes generate automatically.

This single installer consolidates the **best-performing code paths** into one upstream-first system: fewer foot-guns, stronger trust signals, and a clean pipeline that blocks regressions before they ship.
