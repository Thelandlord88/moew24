 ...
 *   geo:smoke           – probe URLs in dist/: from __geo/smoke.seeds.json (or --seeds), else sample from proximity
 ...
 *
 * Usage examples
 *   node scripts/transparent-suite.v2.mjs bootstrap --root .
 *   node scripts/transparent-suite.v2.mjs geo:all --root . --out __geo --strict
 *   node scripts/transparent-suite.v2.mjs geo:smoke --root . --seeds __geo/smoke.seeds.json
 *   node scripts/transparent-suite.v2.mjs vis:ci --root . --out __geo --strict
 *
 * Artifacts
 ...
 *   <out>/smoke.json         – basic existence checks for sampled/seeded pages
...
// ---------- Smoke (now supports seeds) ----------
async function readSeedsFile(p) {
  try {
    ...
    try {
      ...
      // Fallback: parse loose lines (strip quotes, commas, brackets)
      return raw.split(/\r?\n/)
...
async function runSmoke(found) {
  const dist = found.dist && fs.existsSync(found.dist) ? found.dist : null;
  const defaultSeeds = path.join(OUTDIR, 'smoke.seeds.json');
  const seedsArg = ARGV.get('--seeds');
  const seedsPath = seedsArg ? path.resolve(ROOT, seedsArg) : defaultSeeds;

  let urls = null;
  if (fs.existsSync(seedsPath)) {
    urls = await readSeedsFile(seedsPath);
  }

  if (!urls || !urls.length) {
    // Fallback: derive a small sample from proximity.json
  ...
  for (const url of urls) {
    ...
    results.push({ url, exists, file: dist ? rel(file) : '(no dist)'});
  }
  const out = { total: results.length, ok: results.filter(x => x.exists).length, results };
...
async function bootstrap() {
  ...
  add('geo:doctor', 'node scripts/transparent-suite.v2.mjs geo:doctor');
  add('geo:build',  'node scripts/transparent-suite.v2.mjs geo:build');
  add('geo:guard',  'node scripts/transparent-suite.v2.mjs geo:guard');
  add('geo:smoke',  'node scripts/transparent-suite.v2.mjs geo:smoke');
  add('geo:all',    'node scripts/transparent-suite.v2.mjs geo:all --strict');
  add('vis:ci',     'node scripts/transparent-suite.v2.mjs vis:ci --strict');
