...
# 2) Classification probe (pre dynamic import analysis)
if command -v node >/dev/null 2>&1 && [[ -f scripts/ssg-classification-probe.mjs ]]; then
  PROBE=$(node scripts/ssg-classification-probe.mjs || echo '{}')
  echo "Classification probe: $PROBE" | tee -a "$SSR_LOG"
fi

# 3) Detect dangerous dynamic imports (allowlist support)
...
# 4) Check for import assertions (major SSR trigger)
...
# 6) Build test (FAST mode optional)
RUN_BUILD=1
