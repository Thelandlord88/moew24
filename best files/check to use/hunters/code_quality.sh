...
set -euo pipefail
REPORT_COMPLETED=0
trap 'if [[ ! -s "$QUALITY_REPORT" || "$REPORT_COMPLETED" != 1 ]]; then mkdir -p "${REPORT_DIR:-__reports/hunt}"; printf "{\n  \"module\": \"code_quality\", \"status\": \"error\", \"issues\": %s, \"critical\": %s, \"counts\": { \"esmCjsBoundary\": 0 }\n}\n" "${ISSUES:-0}" "${CRITICAL:-0}" > "$QUALITY_REPORT"; fi' EXIT
...
# Allow detection to continue without aborting on empty matches
set +e
...
# Re-enable strict mode before generating JSON
set -e
...
UNUSED_COUNT=$(echo "$UNUSED_IMPORTS" | grep -c . || true)
LONG_FN_COUNT=$(echo "$LONG_FUNCTIONS" | grep -c . || true)
MAGIC_COUNT=$(echo "$MAGIC_NUMBERS" | grep -c . || true)
NESTING_COUNT=$(echo "$DEEP_NESTING" | grep -c . || true)
DUP_COUNT=$(echo "$DUPLICATE_PATTERNS" | grep -c . || true)
LARGE_PARAM_COUNT=$(echo "$LARGE_PARAM_LISTS" | grep -c . || true)
...
cat > "$QUALITY_REPORT" <<EOF
{
  ...
  "findings": {
    ...
    "long_functions": $LONG_FN_COUNT,
    "magic_numbers": $MAGIC_COUNT,
    "deep_nesting": $NESTING_COUNT,
    "todo_markers": ${TODO_COUNT:-0},
    "duplicate_patterns": $DUP_COUNT,
    "large_param_lists": $LARGE_PARAM_COUNT
  },
  "counts": { "esmCjsBoundary": 0 },
  "issues": $ISSUES,
  "critical": $CRITICAL,
  "actions": [ $( (( UNUSED_COUNT>0 )) && echo '"Remove unused imports"') ],
  "policy_invariants": ["counts.esmCjsBoundary==0"],
  "recommendations": [
