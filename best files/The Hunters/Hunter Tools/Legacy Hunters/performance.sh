...
LARGE_IMAGES=$(find src/assets public/ -name "*.png" -o -name "*.jpg" -o -name "*.jpeg" -o -name "*.gif" 2>/dev/null | \
  xargs ls -la 2>/dev/null | awk '$5 > 512000 {print $9 " (" $5 " bytes)"}' || true)
...
BARREL_IMPORTS=$(rg -n "import.*from.*['\"].*index['\"]" src/ --type js --type ts || true)
...
cat > "$PERF_REPORT" <<EOF
{
  ...
  "status": "$STATUS",
  "findings": {
    "large_images": $(echo "$LARGE_IMAGES" | wc -l),
    "png_files": $PNG_COUNT,
    "webp_files": $WEBP_COUNT,
    "total_dependencies": ${DEP_COUNT:-0},
    "large_source_files": $(echo "$LARGE_FILES" | wc -l),
    "barrel_imports": $(echo "$BARREL_IMPORTS" | wc -l)
  },
  "issues": $ISSUES,
  "critical": $CRITICAL,
  "recommendations": [
